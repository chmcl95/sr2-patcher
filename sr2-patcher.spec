...
version is read out of sr2-patcher.py's VERSION line, which the workflow
stamps from the tag before this runs; an unstamped source tree builds as
'dev'.
"""
...
SOURCE = 'sr2-patcher.py'
VERSION = re.search(r"^VERSION = '(.*)'$",
                    pathlib.Path(SOURCE).read_text(encoding='utf-8'),
                    re.M).group(1)

# Windows file properties. Without this the exe carries no version
# information at all, which looks broken in the properties dialog. Each
# component is packed into 16 bits, so anything that is not a dotted number
# in that range gets zeros - which covers 'dev' and also an all-digit short
# SHA, which otherwise looks like a version number and overflows the field.
...
_version_file.write_text("""VSVersionInfo(
  ...
  kids=[
    StringFileInfo([StringTable('040904B0', [
      ...
      StringStruct('FileDescription', 'SR2 Patcher - SEGA RALLY 2 (PC, 1999)'),
      StringStruct('FileVersion', '%(version)s'),
      StringStruct('InternalName', 'sr2-patcher'),
      StringStruct('OriginalFilename', 'sr2-patcher-%(version)s.exe'),
      StringStruct('ProductName', 'SR2 Patcher'),
...
a = Analysis(
    ...
    pathex=[],
    binaries=[],
    datas=[('net/MGNetWk.dll', '.')],   # netplay DLL, read at patch time
    hiddenimports=[],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
