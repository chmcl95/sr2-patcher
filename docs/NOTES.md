...
sixteen fall between `0x440cea`, the join of the replay controls' two
paths, which is where Europe has it, and `0x444bd0`, the processor
check, sixteen back from Europe's `0x444be0`; nothing in the tables sits
between the two. So every site and code address past it - the screen
change, the CD level, the loader's drive scan, the registry open, the
bumpers' page keys, the five volume entries, `RESUME`, and of the HUD
work `TREEDRAW` and `FADEDRAW` - is the European one less `0x10`, and
everything before it, every data address - the volume defaults among
them, in `STATUSDA` - and every import slot, is the European one
unchanged. Every exe site in the row was
...
white - one texel box a letter, at the boxes `HINT_GLYPHS` names on the
English sheet, from the texels `HINT_LETTERING` carries because that
sheet is localised (see *The Options screen*): 17 rows from a row above
each line's ascenders, since the two lines the capitals come from sit a
row lower against their tops, a texel apart, 5 for a space, onto white on the appended sheet at patch
...
- `E_FAIL` at start, through `0x4404b0` from the `jl` at `0x427e05` -
  ...
  deterministic and a build with only `nodisc` and `nocardwarn` applied
  failed identically, so the patches were not in it. That machine's
  cause was the one `anymode` fixes, DirectDraw not listing 640x480 at
  16 bits: with `anymode` in it starts every time and without the shim,
  three runs for three, where before every run failed. Windows' 8/16-bit
  DWM mitigation (`__COMPAT_LAYER=DWM8And16BitMitigation`) cleared it
  there too, which fits - the shim is what makes those modes appear.
  What is still open is the Linux case above, and why that machine had
  one good start before the run of failures.
