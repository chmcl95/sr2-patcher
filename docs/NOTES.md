...
| **Network DLL** (`netplay`, by name, with `lobby`) | `MUSASHI\MGNetWk.dll` | the whole file | replaced by the build of `net/`: the stock DLL's CLSID and three vtables over plain UDP - a LAN search, an address typed, the internet to come; see [NETWORK.md](NETWORK.md) and [net/README.md](../net/README.md) |
